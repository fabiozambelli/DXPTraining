...
public class FooAssetRenderer extends BaseJSPAssetRenderer<Foo> {

	public FooAssetRenderer(Foo foo) {
		_foo = foo;
	}
	...
	public String getSummary(PortletRequest portletRequest, PortletResponse portletResponse) {
		...
		String summary = HtmlUtil.stripHtml(StringUtil.shorten(_foo.getField1(), abstractLength));
		return summary;
	}
	...
	public PortletURL getURLEdit(LiferayPortletRequest liferayPortletRequest,
		...
		Group group = GroupLocalServiceUtil.fetchGroup(_foo.getGroupId());
		if (group.isCompany()) {
		...
		portletURL.setParameter("fooId", String.valueOf(_foo.getFooId()));
	...
	public String getURLViewInContext(LiferayPortletRequest liferayPortletRequest,
		...
		try {
			long plid = PortalUtil.getPlidFromPortletId(_foo.getGroupId(), FooPortletKeys.FOO);
			PortletURL portletURL;
			if (plid == LayoutConstants.DEFAULT_PLID) {
				portletURL = liferayPortletResponse.createLiferayPortletURL(getControlPanelPlid(liferayPortletRequest),
						FooPortletKeys.FOO, PortletRequest.RENDER_PHASE);
			} else {
				portletURL = PortletURLFactoryUtil.getPortletURLFactory().create(liferayPortletRequest,
						FooPortletKeys.FOO, plid, PortletRequest.RENDER_PHASE);
			}
			portletURL.setParameter("mvcRenderCommandName", MVCCommandNames.VIEW_ITEM);
			portletURL.setParameter("fooId", String.valueOf(_foo.getFooId()));
